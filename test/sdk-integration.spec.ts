...
describe('observe pipeline', () => {
  it('exports correlated traces, logs, HTTP/runtime metrics and unified resources', async () => {
    ...
    class Orders {
      ...
      create() { new Logger('Orders').log('created'); }
    }
    new Orders().create();
    const tracer = trace.getTracer('integration');
    tracer.startActiveSpan('GET /orders/:id', {
    ...
    const traceSpan = spans.getFinishedSpans().find((span) => span.name === 'Orders.create');
    expect(traceSpan?.resource.attributes).toMatchObject({
      'service.name': 'mall-api',
      'service.version': '1.2.3',
      'deployment.environment.name': 'test',
      'telemetry.sdk.name': '@ryanzen9/nest-observe',
    });
    expect(traceSpan?.attributes.token).toBe('[REDACTED]');
    const log = logs.getFinishedLogRecords().find((record) => record.body === 'created');
    expect(log?.spanContext?.traceId).toBe(traceSpan?.spanContext().traceId);
    expect(log?.attributes).toMatchObject({ 'nestjs.context': 'Orders', 'service.name': 'mall-api' });
    ...
    expect(metricExporter.getMetrics()[0]?.resource.attributes['service.name']).toBe('mall-api');
  });
});
